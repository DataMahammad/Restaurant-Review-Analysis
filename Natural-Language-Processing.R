...
#Number 1
dataset <- fread("nlpdata.csv")

dataset %>% dim()


dataset %>% glimpse()
dataset$V1 <- dataset$V1 %>% as.character() 
...
split <- dataset$Liked %>% sample.split(SplitRatio = 0.8)
...
#After removing stop words,Max AUC has slightly diminished from 0.918 to 0.912
